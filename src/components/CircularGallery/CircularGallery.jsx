...
class Media {
  ...
  createMesh() {
    ...
    this.plane.userData = { href: this.href || null };
  }
  createTitle() {
...
class App {
  ...
  onClick(e) {
    if (!this.medias || Math.abs(this.scroll.current - (this.scroll.position ?? 0)) > 5) return;
    ...
    this.medias.forEach(media => {
      ...
      if (dist < media.width / 2 && dist < closestDist) {
        closest = media;
        closestDist = dist;
      }
    });
    if (closest?.plane.userData.href) {
      window.open(closest.plane.userData.href, '_blank', 'noopener,noreferrer');
    }
  }
  onWheel(e) {
